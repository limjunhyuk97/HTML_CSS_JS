...
- **생성자 함수, 객체 메소드 제외한 모든 함수 내부의 this**는 **전역 객체(window)를 가리킨다.**
  - 때문에, callback 함수 내부의 this가 해당 함수가 존재하는 객체를 가리키게 하려면 .bind(this) 사용해야 한다.
- **일반 함수** : 함수가 어떻게 호출되었는지에 따라서 this에 바인딩할 객체가 결정된다.
- **화살표 함수** : 자신을 감싼 함수 내의 this를 가리킨다. (선언할 때 정적으로 결정됨)
  - 즉, 화살표 함수는 **렉시컬 스코프를 따른다.**
  - 렉시컬 스코프란, **어디서 선언했는지**를 기준으로 스코프를 결정하는 것이다.
  - **class의 메소드**를 화살표 함수로 정의한다면, 내부의 **this는 상위 컨텍스트인 window**를 가리키게 된다.
  - **prototype의 메소드**를 화살표 함수로 정의한다면, 내부의 **this는 상위 컨텍스트인 window**를 가리키게 된다.
  - **생성자 함수**로 **화살표 함수 사용 불가능**
  - **addEventListener에서 화살표 함수**의 **this는 상위컨텍스트인 window**를 가리키게 된다.
